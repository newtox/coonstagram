<x-guest-layout>
    <div class="min-h-screen flex flex-col items-center justify-center px-4">
        ...
        <div class="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h1 class="text-lg font-bold mb-1">{{ __('coonstagram.verify_email_title') }}</h1>
            <p class="text-sm text-slate-500 mb-6">{{ __('coonstagram.verify_email_text') }}</p>

            @if (session('status') === 'verification-link-sent')
                <div class="mb-4 text-sm text-green-400">{{ __('coonstagram.verify_email_resent') }}</div>
            @endif
